...
const Head = () => {
  ...
  const formatHeadAssignmentSource = (task) => {
    const r = (task.admin || task.role || task.assignedByRole || "").toLowerCase();
    if (r === "ceo" || r === "admin" || r === "superadmin") {
      return task.assignedByName ? `CEO · ${task.assignedByName}` : "CEO / Management";
    }
    if (r === "hr") {
      return task.assignedByName ? `HR · ${task.assignedByName}` : "HR Department";
    }
  ...
  const handleStatusChange = async (task, status) => {
    try {
      ...
      await axios.put(`${API_BASE_URL}/admin/hr_assigned_tasks/${task._id}`, payload);
      setTasks((prev) =>
    ...
    <>
      ...
      >
        ...
        <Box sx={{ width: "98%", mx: "auto", px: { xs: 1, md: 3 }, pt: 4, pb: 10 }}>
          ...
          <Grid container spacing={2} sx={{ mb: 6, width: "100%", justifyContent: "space-between" }}>
            {[
          ...
          <Box sx={{ display: "flex", gap: 3, mb: 8, flexWrap: "wrap", justifyContent: "space-between", width: "100%" }}>
            {[
              ...
              { label: "Custom", icon: <DashboardCustomizeIcon />, onClick: handleOpenCustomDialog },
              { label: "Everything", icon: <AssessmentIcon />, onClick: () => setOpenEverything(true) },
            ].map((action, idx) => (
              <Button
                key={idx}
                variant="outlined"
                ...
                <Grid item xs={12} md={6} lg={4} key={task._id}>
                  ...
                  >
                    <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2, alignItems: "flex-start" }}>
                      <Typography sx={{ fontWeight: 800, color: "#0f172a", fontSize: "1.25rem", lineHeight: 1.2, letterSpacing: "-0.02em" }}>
                        {task.title}
                      </Typography>
                      <Chip
                    ...
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5, mb: 3 }}>
                      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                          ...
                          <Typography sx={{ color: "#64748b", fontSize: "0.85rem", fontWeight: 500 }}>Deadline</Typography>
                        </Box>
                        <Typography sx={{ color: "#1e293b", fontSize: "0.85rem", fontWeight: 700 }}>
                          {task.deadline ? new Date(task.deadline).toLocaleDateString("en-GB", { day: '2-digit', month: 'short' }) : "-"}
                        </Typography>
                      </Box>
                      ...
                      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                          ...
                          <Typography sx={{ color: "#64748b", fontSize: "0.85rem", fontWeight: 500 }}>Source</Typography>
                        </Box>
                        <Chip
                          ...
                          sx={{
                            ...
                            borderRadius: "4px",
                            ...(() => {
                              const role = (task.admin || task.role || task.assignedByRole || "").toLowerCase();
                              if (role === "hr") {
                                return { bgcolor: "#ecfdf5", color: "#065f46", border: "1px solid #d1fae5" };
                              }
                              if (role === "ceo" || role === "admin" || role === "superadmin") {
                                return { bgcolor: "#eff6ff", color: "#1e40af", border: "1px solid #dbeafe" };
                              }
